...
naughty.config.paddingx = 4
naughty.config.paddingy = 4
...
local function get_offset(screen, position, idx, width, height)
    ...
    if position:match("left") then
        v.x = ws.x + naughty.config.paddingx
--  ++++
    elseif position:match("center") then
        v.x = ws.x + ( ws.width - (width + naughty.config.paddingx) ) / 2
--  ++++
    else
        v.x = ws.x + ws.width - (width + naughty.config.paddingx)
    end
    ...
    if position:match("top") then
        v.y = ws.y + naughty.config.paddingy + existing
    else
        v.y = ws.y + ws.height - (naughty.config.paddingy + height + existing)
    end
...
function naughty.notify(args)
    ...
    if width > workarea.width - 2 * (border_width or 0) - 2 * (naughty.config.paddingx or 0) then
        width = workarea.width - 2 * (border_width or 0) - 2 * (naughty.config.paddingx or 0)
    end
    if height > workarea.height - 2 * (border_width or 0) - 2 * (naughty.config.paddingy or 0) then
        height = workarea.height - 2 * (border_width or 0) - 2 * (naughty.config.paddingy or 0)
    end
